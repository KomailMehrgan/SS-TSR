...
class UpscaleNetwork(nn.Module):
    def __init__(self, upscale_factor=4, num_channels=64):
        ...
        if self.upscale_factor == 2:
            self.upscale = nn.Sequential(
                nn.Conv2d(in_channels=num_channels, out_channels=num_channels * 4, kernel_size=3, stride=1, padding=1,
                          bias=False),
                nn.PixelShuffle(2),
...
class _NetG(nn.Module):
    def __init__(self, num_channels=64):
        ...
        self.conv_input = nn.Conv2d(in_channels=3, out_channels=num_channels, kernel_size=9, stride=1, padding=4,
                                    bias=False)
        self.relu = nn.LeakyReLU(0.2, inplace=True)

        self.residual = self.make_layer(_Residual_Block, 32, num_channels)

        self.conv_mid = nn.Conv2d(in_channels=num_channels, out_channels=num_channels, kernel_size=3, stride=1,
                                  padding=1, bias=False)
        self.bn_mid = nn.InstanceNorm2d(num_channels, affine=True)

        self.upscalex = UpscaleNetwork(upscale_factor=2, num_channels=num_channels)

        self.conv_output = nn.Conv2d(in_channels=num_channels, out_channels=3, kernel_size=9, stride=1, padding=4,
                                     bias=False)
