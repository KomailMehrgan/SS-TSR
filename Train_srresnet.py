...
from Network.srresnet import _NetG, _NetD
...
def main():
    ...
    # Create a parent folder with the current date and time
    start_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    parent_folder = os.path.join("back_up_models/SR/RES", start_time,"SRresnet")
    ...
    sr_data_path = "datasets/data"  # Replace with your HR image directory
    ...
    # Send models and losses to GPU if available
    if device.type == "cuda":
        netSR = netSR.to(device)
        mse_criterion = mse_criterion.to(device)
        entropy_criterion = entropy_criterion.to(device)

    # Load checkpoint or pretrained model if provided
    if opt.resume or opt.pretrained:
        weights_path = opt.resume or opt.pretrained
        if os.path.isfile(weights_path):
            print(f"=> loading model from '{weights_path}'")
            checkpoint = torch.load(weights_path,map_location=torch.device('cuda'),weights_only=False)
            netSR.load_state_dict(checkpoint['model'].state_dict())
        else:
            print(f"=> no model found at '{weights_path}'")

    print("===> Setting Optimizer")
    # optimizerG = optim.Adam(netSR.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    optimizerG = torch.optim.AdamW(netSR.parameters(), lr=0.0001, weight_decay=1e-5)
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        test_one_image(netSR, epoch, parent_folder)

def train(data_loader, optimizerG, netSR, netOCR, mse_criterion, epoch, metrics, converter, parent_folder, accumulation_steps=8):
    ...
    for iteration, batch in enumerate(data_loader, 1):
        ...
        img_loss = mse_criterion(realImage, SrImage)
        if SrImage.shape[1] == 3:
            SrImage_img_ocr = SrImage.mean(dim=1, keepdim=True)
