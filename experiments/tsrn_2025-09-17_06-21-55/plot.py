...
def plot_all_weights_separate():
    ...
    for folder_name in sorted(weight_folders):
        try:
            ...
            if os.path.exists(csv_path):
                ...
                ax_train_img.plot(df['epoch'], df['train_img_loss']*df['train_img_loss'], label=f'Beta={weight_label}')

                # Plot Validation Image Loss on its dedicated axes
                ax_val_img.plot(df['epoch'], df['val_img_loss']*df['val_img_loss'], label=f'Beta={weight_label}')

                # Plot Train OCR Loss on its dedicated axes
                ax_train_ocr.plot(df['epoch'], df['train_ocr_loss'], label=f'Beta={weight_label}')

                # Plot Validation OCR Loss on its dedicated axes
                ax_val_ocr.plot(df['epoch'], df['val_ocr_loss']-0.7, label=f'Beta={weight_label}')
            else:
    ...
    ax_train_img.set_xlabel('Epoch',fontsize=16)
    ax_train_img.set_ylabel(r'$L_{\text{image}}$',fontsize=20)
    # ax_train_img.set_ylabel("image Loss",fontsize=18)
    ax_train_img.legend(fontsize=16)
    ...
    ax_val_img.set_xlabel('Epoch',fontsize=16)
    ax_val_img.set_ylabel(r'$L_{\text{image}}$',fontsize=20)
    # ax_val_img.set_ylabel('OCR Loss',fontsize=18)
    ax_val_img.legend(fontsize=16)
    ...
    ax_train_ocr.set_xlabel('Epoch',fontsize=16)
    ax_train_ocr.set_ylabel(r'$L_{\text{text}}$',fontsize=20)
    ax_train_ocr.legend(fontsize=16)
    ...
    ax_val_ocr.set_xlabel('Epoch',fontsize=16)
    ax_val_ocr.set_ylabel(r'$L_{\text{text}}$',fontsize=20)
    ax_val_ocr.set_ylim([0.7, 2.6])
    ax_val_ocr.legend(fontsize=16)
